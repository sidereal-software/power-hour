 ...
 */
import { getClientId, redirectUri, SCOPES } from './config'
...
function writeTokens(payload: TokenResponse, existing: StoredTokens | null): StoredTokens {
...
export type RedirectResult = 'signed-in' | 'denied' | null

/**
 * Consume `?code=…` if we just came back from Spotify.
 * Always leaves a clean URL behind.
 */
export async function handleRedirect(): Promise<RedirectResult> {
  ...
  if (!state || state !== expectedState) {
    throw new Error('Auth state mismatch — please try connecting again.')
  }
...
export function forceRefresh(): Promise<StoredTokens> {
